"""Base utilities for Ollama research tools."""
...
def chat(prompt: str, system: str = None, model: str = None) -> str:
    ...
    try:
        with console.status(f"[bold cyan]Analyzing with {model}...[/bold cyan]"):
            response = ollama.chat(model=model, messages=messages)
        return response['message']['content']
    except Exception as e:
        return f"Error: {e}"

def get_multiline_input(prompt_text: str = "Enter text (type 'END' when done):") -> str:
...
def display_result(title: str, content: str, style: str = "green"):
    console.print()
    console.print(Panel(Markdown(content), title=f"[bold {style}]{title}[/bold {style}]", border_style=style))
